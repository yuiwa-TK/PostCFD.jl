...
"""
function read_dividedrestart(Compdir::String,flowids::Union{OrdinalRange,Vector{Int}})
...
function read_dividedrestart(Compdir::String,flowids::Union{OrdinalRange,Vector{Int}},filebase::String)
    vec_flow = Vector{Array{AbstractFloat}}(undef,length(flowids))
    count = 0
    for n in flowids
        divide_num  = @sprintf "%5.5i"  n
...
function read_dividedsnapshot(Compdir::String,flowids::Union{OrdinalRange,Vector{Int}},
                            filebase::String,suffix::String)

    vec_flow =  Vector{Array{AbstractFloat}}(undef,length(flowids))
    count = 0
    for n in flowids
        divide_num  = @sprintf "%8.8i"  n
        flowfile    = Compdir*filebase*divide_num
        println("read file ",flowfile)
        count+=1
        vec_flow[count]=read_restart(flowfile)
...
"""
function read_dividedgrid(Compdir::String,gridids::Union{OrdinalRange,Vector{Int}})
    # OrdinalRange includes typeof(ns:ne) and typeof(ns:nskip:ne) 
    # gridids=ns:nskip:ne
