...
"""
    derivative_curvilinear!(df::AbstractArray{S},f::AbstractArray{S},metrics::AbstractArray{T},func_deriv::Function,idirection::Int) where {S, T}
is a mutation variant of derivative_curvilinear, in which the first argument `df`` is updated.

- Forth argument func_deriv! should be a mutation function.
"""
function derivative_curvilinear!(df::AbstractArray{S},f::AbstractArray{S},metrics::AbstractArray{T},func_deriv!::Function,idirection::Int) where {S, T}

    # df/dξ, df/dη, df/dζ
    dfdξ, dfdη, dfdζ = similar(f), similar(f), similar(f)
    for l in axes(f,3), k in axes(f,2)
        dfdξ[:,k,l] = func_deriv!(view(f,:,k,l),dfdξ[:,k,l])
    end
    for l in axes(f,3), j in axes(f,1)
        dfdη[j,:,l] = func_deriv!(view(f,j,:,l),dfdξ[j,:,l])
    end
    for k in axes(f,2), j in axes(f,1)
        dfdζ[j,k,:] = func_deriv!(view(f,j,k,:),dfdξ[j,k,:])
    end
